...
const ScatterPlot = ( props ) => {
    ...
    let onMouseOver = ( event ) => {
        Graph.drawControls( ref, width, height, margin, padding, true, false, false, xScale, yScale, xDomain0, yDomain0, xLabel, yLabel );
    };
    let onMouseOut = ( event ) => {
        let xUp = event.nativeEvent.offsetX,
            yUp = event.nativeEvent.offsetY,
            isZooming = (( 0 <= xUp ) && ( xUp < width ) && ( 0 <= yUp ) && ( yUp < height ));
        Graph.drawControls( ref, width, height, margin, padding, isZooming, false, false, xScale, yScale, xDomain0, yDomain0, xLabel, yLabel );
    };
